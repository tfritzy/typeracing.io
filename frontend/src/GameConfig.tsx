...
let modes: Record<ValidGameMode, ModeConfig> = {
 ...
 HomeRow: {
  name: "Home row",
  description:
   "Words that can be typed using only the home row.",
 ...
 UpperRow: {
  name: "Upper row",
  description:
   "Words that can be typed using only the upper row.",
 ...
 LeftHand: {
  name: "Left hand only",
  description:
   "Words that can be typed using only the left hand.",
  icon: (
 ...
 RightHand: {
  ...
  description:
   "Words that can be typed using only the right hand.",
 ...
 AlternatingHand: {
  ...
  description:
   "Words that alternate between left and right hand.",
...
const Content = () => {
 ...
 const gameType = useSelector(
  (state: RootState) => state.player.gameType
 );
...
export const GameConfig = (props: GameConfigProps) => {
 ...
 let modesString = `${
  modes[player.gameMode as ValidGameMode].name
 }`;

 return (
  <>
   <button
    className="flex flex-row items-center border border-border-color space-x-2 rounded-full p-2 px-4 text-secondary focus:text-main bg-neutral-color"
