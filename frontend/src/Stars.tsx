...
const generateStar = (rng: ReturnType<typeof splitmix32>, x?: number) => {
  const U = rng.next();
  const lambda = 1;
...
export const Stars: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const drawScene = () => {
      const deltaTime_s: number = (Date.now() - lastTime) / 1000;
      lastTime = Date.now();

      const starBaseMovementSpeed: number =
        speedRef.current?.valueAsNumber || 0.3;
      stars.forEach((star) => {
        star.speed =
          star.z * starBaseMovementSpeed + starBaseMovementSpeed / 40;
      ...
      for (let star of stars) {
        const x1 = star.x - star.size * 0.001 * Math.max(star.speed, 0.1) * 10;
        const x2 = star.x + star.size * 0.001 * Math.max(star.speed, 0.1) * 10;
