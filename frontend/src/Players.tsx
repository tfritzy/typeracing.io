...
const Ship = () => {
  return <img src="/ship.svg" alt="Ship" className="w-16 h-16 rotate-90" />;
};
...
const PlayerRow = (player: PlayerProps) => {
  return (
    <div className="h-md w-screen px-8 relative">
      ...
      <div
        className="absolute"
        style={{ left: (player.position_km / screenWidthKm) * 100 + "%" }}
      >
        <Ship />
      </div>
    </div>
...
export const Players = () => {
  ...
  const self = players.find((player) => player.id === playerData.id);
  const [positionDeltas, setPositionDeltas] = React.useState<number[]>([]);
